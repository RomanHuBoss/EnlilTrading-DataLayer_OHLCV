...
def _mk_df(n: int = 2000) -> pd.DataFrame:
    ...
    df["f_close_z_60"] = (df["close"] - df["close"].rolling(60, min_periods=1).mean())/df["close"].rolling(60, min_periods=1).std(ddof=1).fillna(1.0)
    df["f_adx14"] = 25.0
    df["f_pdi14"] = 20.0
    df["f_mdi14"] = 10.0
...
def test_infer_smoke():
    ...
    # Проверки столбцов
    req = {
        "regime", "high_rv", "regime_confidence", "votes_trend", "votes_flat",
        "det_used", "chgpt", "p_bocpd_trend", "p_bocpd_flat", "s_adx", "s_donch", "s_hmm_rv",
        "hysteresis_state", "symbol", "tf", "build_version"
    }

...
def validate_1m_index(df: pd.DataFrame) -> None:
    """Проверка: tz-aware UTC, кратность минуте, монотонность, отсутствие дубликатов."""
...
def missing_rate(df: pd.DataFrame, freq: str = "T") -> float:
    """Доля пропусков относительно полной сетки."""
...
def ensure_missing_threshold(df: pd.DataFrame, threshold: float = 0.0001) -> None:
    """Исключение при превышении доли пропусков."""

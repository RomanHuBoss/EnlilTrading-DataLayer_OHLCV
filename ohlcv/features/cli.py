...
def main(argv=None):
    p = argparse.ArgumentParser(prog="features-core", description="C3 Features.Core — build features from OHLCV CSV")
    sub = p.add_subparsers(dest="cmd", required=True)

    b = sub.add_parser("build", help="build features parquet from CSV")
    b.add_argument("--input", required=True, type=Path)
    b.add_argument("--symbol", required=True)
    b.add_argument("--tf", required=True)
    b.add_argument("--config", type=Path, default=None, help="YAML with parameters (optional)")
